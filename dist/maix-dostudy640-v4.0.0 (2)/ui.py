...
# ==============================================================================
# 8. TouchKeyboard Component
# ==============================================================================
class TouchKeyboard:
    """触摸屏软键盘，用于在设备上输入简短文本（如人脸标签名）。

    支持字母、数字、退格和确认，整体覆盖在当前画面上层。
    使用方式：
        kb = TouchKeyboard(ts, disp, initial_text="")
        # 每帧调用：
        done, text = kb.handle_events(img)
        if done is True:   # 用户确认
            use(text)
        elif done is False: # 用户取消
            cancel()
    """
    ...
    def _build_key_rects(self):
        """预计算所有按键的位置（图像坐标）。"""
        rows = self._ROWS
        rects = []  # list of (label, [x, y, w, h])

        total_rows = len(rows)
        row_h = self._KB_H // total_rows
        pad = 3

        for ri, row in enumerate(rows):
            y = self._KB_Y + ri * row_h
            n = len(row)
            if ri == len(rows) - 1:
                # 最后一行：取消 / 确认 各占一半
                btn_w = self._KB_W // 2 - pad
                for ci, label in enumerate(row):
                    x = self._KB_X + ci * (self._KB_W // 2)
                    rects.append((label, [x + pad, y + pad, btn_w - pad, row_h - pad * 2]))
            else:
                btn_w = self._KB_W // n
                for ci, label in enumerate(row):
                    x = self._KB_X + ci * btn_w
                    rects.append((label, [x + pad, y + pad, btn_w - pad * 2, row_h - pad * 2]))
        return rects

    def _hit_key(self, tx: int, ty: int, img_w: int, img_h: int) -> str | None:
        """根据显示坐标判断命中哪个键，返回标签或 None。"""
        disp_w, disp_h = self._disp.width(), self._disp.height()
        for label, rect in self._key_rects:
            disp_rect = image.resize_map_pos(
                img_w, img_h, disp_w, disp_h,
                image.Fit.FIT_CONTAIN, *rect)
            if (disp_rect[0] < tx < disp_rect[0] + disp_rect[2] and
                    disp_rect[1] < ty < disp_rect[1] + disp_rect[3]):
                return label
        return None
    ...
    def draw(self, img: image.Image):
        """在 img 上绘制软键盘覆盖层。"""
        # 半透明背景（用深色矩形模拟）
        img.draw_rect(0, self._KB_Y - 44, img.width(), img.height() - (self._KB_Y - 44),
                      color=image.Color.from_rgb(20, 20, 20), thickness=-1)

        # 输入框
        input_text = self.text if self.text else ' '
        img.draw_rect(self._KB_X, self._KB_Y - 40, self._KB_W, 36,
                      color=image.Color.from_rgb(50, 50, 80), thickness=-1)
        img.draw_rect(self._KB_X, self._KB_Y - 40, self._KB_W, 36,
                      color=image.Color.from_rgb(100, 150, 255), thickness=2)
        display_text = input_text[-28:] if len(input_text) > 28 else input_text
        img.draw_string(self._KB_X + 8, self._KB_Y - 34,
                        display_text + '|',
                        color=image.Color.from_rgb(255, 255, 255), scale=1.3)

        # 绘制按键
        for label, rect in self._key_rects:
            is_ctrl = label in ('取消', '确认', '⌫')
            if label == '确认':
                bg = image.Color.from_rgb(0, 120, 60)
            elif label == '取消':
                bg = image.Color.from_rgb(120, 30, 30)
            elif label == '⌫':
                bg = image.Color.from_rgb(80, 60, 30)
            else:
                bg = image.Color.from_rgb(55, 55, 75)

            img.draw_rect(*rect, color=bg, thickness=-1)
            img.draw_rect(*rect, color=image.Color.from_rgb(90, 90, 110), thickness=1)

            scale = 1.1 if is_ctrl else 1.2
            ts = image.string_size(label, scale=scale)
            tx = rect[0] + (rect[2] - ts.width()) // 2
            ty = rect[1] + (rect[3] - ts.height()) // 2
            img.draw_string(tx, ty, label,
                            color=image.Color.from_rgb(230, 230, 230), scale=scale)
    ...
    def handle_events(self, img: image.Image):
        ...
        if pressed:
            key = self._hit_key(x, y, img_w, img_h)
            if key and self._armed_key is None:
                self._armed_key = key
        else:
