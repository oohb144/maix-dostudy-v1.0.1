...
class AudioController:
    ...
    def _play_wav_thread(self, file_path):
        ...
        用 block=False 播放器 + 分块喂 PCM 的方式，每个 play(chunk)
        调用迅速返回、让出 GIL，避免阻塞主线程导致画面卡顿。
        """
        try:
            ...
            # 解析 WAV 头，得到采样率/声道/位宽及 data 块偏移
            with open(file_path, "rb") as f:
                header = f.read(64)

            if len(header) < 44 or header[0:4] != b"RIFF" or header[8:12] != b"WAVE":
                print(f"[音频] WAV 头无效，无法流式播放: {file_path}")
                return

            channel = int.from_bytes(header[22:24], "little") or 1
            sample_rate = int.from_bytes(header[24:28], "little") or 16000
            bits_per_sample = int.from_bytes(header[34:36], "little") or 16

            data_offset = 44
            if header[36:40] != b"data":
                for idx in range(12, len(header) - 8):
                    if header[idx:idx + 4] == b"data":
                        data_offset = idx + 8
                        break

            # 创建非阻塞播放器（block=False，play() 立即返回）
            player = audio.Player(sample_rate=sample_rate, channel=channel, block=False)
            player.volume(80)

            # 按帧对齐块大小，避免拆到半帧
            bytes_per_sample = max(1, bits_per_sample // 8)
            align = max(1, channel * bytes_per_sample)
            chunk_bytes = 8192
            ...
            with open(file_path, "rb") as f:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    valid = (len(chunk) // align) * align
                    if valid == 0:
                        continue
                    chunk = chunk[:valid]

                    # 等待底层缓冲有空闲再写入（这也让出 GIL）
                    if hasattr(player, "get_remaining_frames") and hasattr(player, "frame_size"):
                        frame_size = player.frame_size()
                        need_frames = int((len(chunk) + frame_size - 1) / frame_size)
                        while True:
                            idle_frames = player.get_remaining_frames()
                            if idle_frames >= need_frames:
                                break
                            time.sleep_ms(4)
            ...
            # 等待底层缓冲尽量输出完成，再释放
            if hasattr(player, "period_count") and hasattr(player, "period_size"):
                try:
                    total_frames = player.period_count() * player.period_size()
                    while True:
                        if player.get_remaining_frames() >= total_frames:
                            break
                        time.sleep_ms(10)
                except Exception:
                    pass
