...
class FaceRecognitionUI:
    ...
    def _handle_recognizing(self):
        ...
        if detected_faces:
            ...
            # 识别完成、转入录制前播放音效（此时录音器未启动，设备空闲）
            # 仅在人脸类型变化时触发一次，避免重复播放
            face_type = 'known' if has_known else 'unknown'
            if self._last_face_type != face_type:
                if has_known:
                    self._audio_play('play_wav_file', file_path=SOUND_KNOWN)
                else:
                    self._audio_play('play_wav_file', file_path=SOUND_STRANGER)
    ...
    def _handle_recording(self):
        ...
        if detected_faces:
            ...
            # 人脸类型变化时：串口通知（各触发一次）
            # 注：音效在 RECOGNIZING 状态播放，避免与录音抢占音频设备
            face_type = 'known' if has_known else 'unknown'
            if self._last_face_type != face_type:
        ...
        try:
            ...
            if start_result:
                ...
                if success:
                    ...
                    self._audio_play('play_wav_file', file_path=SOUND_ENROLL_SUCCESS)
                    self._led_blink(100)
